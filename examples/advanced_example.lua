...
local function example_size_limits()
  ...
  local callbacks = {
    ...
    on_part_data = function(data)
      ...
      if limiter.current_part_bytes > limiter.max_part_bytes then
        print(string.format("  Part size limit exceeded: %d > %d", limiter.current_part_bytes, limiter.max_part_bytes))
      ...
      if limiter.total_bytes > limiter.max_total_bytes then
        print(string.format("  Total size limit exceeded: %d > %d", limiter.total_bytes, limiter.max_total_bytes))
        limiter.limit_exceeded = true
        return 1 -- Pause parsing
      end
  ...
  print(
    string.format("Parsing with limits: max_part=%d, max_total=%d", limiter.max_part_bytes, limiter.max_total_bytes)
  )
...
local function example_streaming_boundary_conditions()
  ...
  local callbacks = {
    ...
    on_part_data = function(data)
      ...
      print(string.format("  Data callback #%d: %d bytes", state.data_callbacks, #data))
      return 0
    end,
  }
  ...
  print(string.format("\nSuccessfully parsed %d parts with split boundaries", state.part_count))
  print(string.format("Parser correctly handled %d data callbacks", state.data_callbacks))
...
local function example_complete_application()
  ...
  local data = "------WebKitFormBoundary7MA4YWxkTrZu0gW\r\n"
  ...
  local callbacks = {
    ...
    on_part_data_end = function()
      local part_info = string.format("field='%s'", state.current_field_name or "(unknown)")

      if state.has_filename then
        part_info = part_info
          .. string.format(", filename='%s', size=%d bytes", state.current_filename, state.current_part_size)
        state.file_count = state.file_count + 1
      else
        part_info = part_info .. string.format(", size=%d bytes", state.current_part_size)
      end
